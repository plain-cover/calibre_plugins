...
**A browser window may open during downloads.** The plugin tries the Romance.io JSON API first, which is fast and requires no browser. If the API is unavailable, it falls back to opening a Chrome browser window to scrape the page directly - just ignore it, it will close automatically. JSON API lookups are fast; browser-based lookups take ~10–30 seconds per book.

**Chrome is required for the browser fallback.** If Chrome is not installed, the plugin can only use the JSON API. Install Chrome from [google.com/chrome](https://www.google.com/chrome/). On Apple Silicon Macs (M1/M2/M3/M4), Chrome's browser automation also requires Rosetta 2 - if it's missing, the plugin's job log will tell you how to install it.
