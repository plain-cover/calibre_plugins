...
**A browser window may open during downloads.** The plugin tries the Romance.io JSON API first, which is fast and requires no browser. If the API doesn't return a result, it will open a browser window, which you should just ignore - it will close automatically. Speed depends on whether the browser is needed. JSON API lookups are fast. Browser-based lookups can take ~10–30 seconds per book. Multiple books are processed one at a time.
